...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
        ...
        if world.dataset == 'lastfm':
            ...
            self.Graph,self.Graph_FA, self.Graph_WF, self.Graph_side = self.dataset.getSparseGraph()
        ...
        self.HFAAlayer = self.config['HFAAlayer']
    ...
    def computer(self):
        ...
        for layer in range(self.n_layers):#遍历每一层
            if layer >= self.n_layers-self.PLNSlayer and self.use_WF == 'True':#if layer >= 2 and self.use_WF == True:
                ...
                embs.append(all_emb)
            if layer >= self.n_layers-self.HFAAlayer and self.use_FA == 'True':
